...
describe("Dtweet", function () {
  ...
  beforeEach(async () => {
    ...
    // Deploy contract
    dtweet = await DtweetFactory.deploy();
    // user1 mints an nfts 
    await dtweet.connect(user1).mint(URI)
  })
  describe('Deployment', async () => {
    it("Should track name and symbol", async function () {
      const nftName = "Dtweet"
      const nftSymbol = "DTWEET"
      expect(await dtweet.name()).to.equal(nftName);
      expect(await dtweet.symbol()).to.equal(nftSymbol);
    });
  })
  describe('Minting NFTs', async () => {
    it("Should track each minted NFT", async function () {
      expect(await dtweet.tokenCount()).to.equal(1);
      expect(await dtweet.balanceOf(user1.address)).to.equal(1);
      expect(await dtweet.tokenURI(1)).to.equal(URI);
      // user2 mints an nft 
      await dtweet.connect(user2).mint(URI)
      expect(await dtweet.tokenCount()).to.equal(2);
      expect(await dtweet.balanceOf(user2.address)).to.equal(1);
      expect(await dtweet.tokenURI(2)).to.equal(URI);
    });
  })
  describe('Setting profiles', async () => {
    it("Should allow users to select which NFT they own to represent their profile", async function () {
      ...
      expect(await dtweet.profiles(user1.address)).to.equal(2);
      // user 1 sets profile to first minted nft
      await dtweet.connect(user1).setProfile(1)
      expect(await dtweet.profiles(user1.address)).to.equal(1);
      // FAIL CASE //
      // user 2 tries to set their profile to nft number 2 owned by user 1
      await expect(
        dtweet.connect(user2).setProfile(2)
  ...
  describe('Uploading posts', async () => {
    it("Should track posts uploaded only by users who own an NFT", async function () {
      ...
      await expect(dtweet.connect(user1).uploadPost(postHash))
        .to.emit(dtweet, "PostCreated")
        .withArgs(
          1,
          postHash,
          0,
      ...
      const postCount = await dtweet.postCount()
      expect(postCount).to.equal(1);
      // Check from struct
      const post = await dtweet.posts(postCount)
      ...
      await expect(
        dtweet.connect(user2).uploadPost(postHash)
      ).to.be.revertedWith("Must own a dtweet nft to post");
      // FAIL CASE #2 //
      // user 1 tried to upload a post with an empty post hash.
      await expect(
        dtweet.connect(user1).uploadPost("")
  ...
  describe('Tipping posts', async () => {
    it("Should allow users to tip posts and track each posts tip amount", async function () {
      // user1 uploads a post
      await dtweet.connect(user1).uploadPost(postHash)
      ...
      await expect(dtweet.connect(user2).tipPostOwner(1, { value: tipAmount }))
        .to.emit(dtweet, "PostTipped")
        .withArgs(
          1,
          postHash,
          tipAmount,
      ...
      const post = await dtweet.posts(1)
      ...
      await expect(
        dtweet.connect(user2).tipPostOwner(2)
      ).to.be.revertedWith("Invalid post id");
      // FAIL CASE #2 //
      // user 1 tries to tip their own post
      await expect(
        dtweet.connect(user1).tipPostOwner(1)
